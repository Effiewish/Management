...
public abstract class PageInforService {
	...
	public PageInforBean getPageListByPageNo(PageInforBean pageInforBean)
	{
		...
		//根据检索条件，查询满足条件的总记录数
		int totalNumber = getTotalRecordNumber(queryInforMap);//14
		int showCount = pageInforBean.getShowCount();  //5
		//根据总记录数和每页显示的记录数，获取总页数；
		int tmp = totalNumber% showCount; //14%5
		...
		int endCount = pageInforBean.getFromCount()+showCount-1;//6+5-1=10
